...
#include <string.h>
#include "../include/AudioResamplerSinc.h"
 ...
 */
    const uint32_t AudioResamplerSinc::mFirCoefsUp[] = {
 ...
 */
    const uint32_t AudioResamplerSinc::mFirCoefsDown[] = {
                                      ...
                                      AudioBufferProvider* provider)
    {
        ...
        while (outputIndex < outputSampleCount) {
            ...
            while (buffer.frameCount == 0) {
                ...
                int32_t  status = provider->getNextBuffer(&buffer);
                //ALOGE("AudioResamplerSinc Status : %d", status);
                if (buffer.raw == NULL) {
                    goto resample_exit;
                }
                const uint32_t phaseIndex = phaseFraction >> kNumPhaseBits;
                //ALOGE("AudioResamplerSinc phaseIndex : %d", phaseIndex);
                if(phaseIndex == 0){
                    break;
...
**/
            ...
            int32_t& l, int32_t& r, uint32_t phase, int16_t const *samples)
    {
        ...
        l = 0;
        r = 0;
        int32_t const* coefs = (int32_t*)mFirCoefs;
