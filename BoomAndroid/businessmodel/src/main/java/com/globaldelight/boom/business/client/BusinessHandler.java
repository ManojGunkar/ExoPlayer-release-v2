...
public class BusinessHandler {
    ...
    private static final String ACTION_REGISTER_ANDROID_DEVICE = "is_device_registered";

    private BusinessHandler(Context context){
        mContext = context;
        networkCallHandler = NetworkCalls.getNetworkCallsInstance(context);
        shp = mContext.getSharedPreferences("business_model", mContext.MODE_PRIVATE);
    ...
    public void registerAndroidDevice(){
        if(shp.getBoolean(ACTION_REGISTER_ANDROID_DEVICE, false)) {
            networkCallHandler.registerDevice();
            shp.edit().putBoolean(ACTION_REGISTER_ANDROID_DEVICE, true).apply();
        }
    }

    public void getConfigAppWithBoomServer(){
    ...
    public void isAppTrialVersion(){
        networkCallHandler.isAppTrailVersion();
    }
