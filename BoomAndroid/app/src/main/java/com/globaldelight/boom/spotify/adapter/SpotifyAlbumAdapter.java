...
public class SpotifyAlbumAdapter extends RecyclerView.Adapter<SpotifyAlbumAdapter.AlbumViewHolder> {
    ...
    public void onBindViewHolder(AlbumViewHolder holder, int position) {
        if (list.get(position).getImages().get(0).getUrl() != null) {
            Picasso.with(context).load(list.get(position).getImages().get(0).getUrl())
                    .resize(1000, 500)
                    .onlyScaleDown()
   ...
   class AlbumViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public AlbumViewHolder(View itemView) {
            super(itemView);
            imgAlbumThumb = (ImageView) itemView.findViewById(R.id.img_album_thumb_spotify);
            txtAlbumTitle = (TextView) itemView.findViewById(R.id.txt_album_title_spotify);
